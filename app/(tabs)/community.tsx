...
export default function CommunityScreen() {
  ...
  const openComments = async (reportId: number) => {
    setCommentsReportId(reportId); setCommentsLoading(true); setComments([]);
    try {
  ...
  const closeComments = () => { setCommentsReportId(null); setComments([]); setCommentText(''); };
  ...
  const submitComment = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setCommentText('');
        setReports(prev => prev.map(r => r.id === commentsReportId ? { ...r, comments_count: (r.comments_count || 0) + 1 } : r));
      }
    } catch (e) { console.error(e); }
  ...
  const renderFeedCard = ({ item }: { item: any }) => {
    ...
    return (
      <TouchableOpacity activeOpacity={0.85} onPress={() => router.push(`/report/${item.id}`)} style={styles.feedCard}>
        ...
        {item.image_url ? (
          <Image source={{ uri: fixImageUrl(item.image_url) ?? undefined }} style={styles.feedImage} contentFit="cover" />
        ) : null}
        <View style={styles.feedStats}>
          <TouchableOpacity style={styles.statBtn} onPress={(e) => { e.stopPropagation(); handleLike(item.id); }}>
            <Ionicons name={isLiked ? 'thumbs-up' : 'thumbs-up-outline'} size={16} color={isLiked ? COLORS.primary : COLORS.textSub} />
            <ThemedText style={[styles.statText, isLiked && styles.statTextActive]}>{item.likes_count || 0}</ThemedText>
          </TouchableOpacity>
    ...
    <View style={styles.sectionContainer}>
      ...
      <View style={styles.filterWrapper}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterScroll}>
          <TouchableOpacity style={[styles.filterChip, selectedLine === '전체' && styles.activeFilterChip]} onPress={() => setSelectedLine('전체')}>
          ...
          {SUBWAY_LINES.filter(l => l !== '전체').map(line => (
            <TouchableOpacity key={line} style={[styles.filterChip, selectedLine === line && styles.activeFilterChip]} onPress={() => setSelectedLine(line)}>
            ...
            return (
              <TouchableOpacity key={item.id} activeOpacity={0.85} onPress={() => router.push(`/report/${item.id}`)} style={styles.popularCard}>
                ...
                <View style={styles.cardStats}>
                  <TouchableOpacity style={styles.statBtn} onPress={(e) => { e.stopPropagation(); handleLike(item.id); }}>
                    <Ionicons name={isLiked ? 'thumbs-up' : 'thumbs-up-outline'} size={14} color={isLiked ? COLORS.primary : COLORS.textSub} />
                    <ThemedText style={[styles.statText, isLiked && styles.statTextActive]}>{item.likes_count || 0}</ThemedText>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.statBtn} onPress={() => router.push(`/report/${item.id}`)}>
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        <Image source={require('@/assets/images/character_design.svg')} style={{ width: 36, height: 36 }} contentFit="contain" />
      ...
      />
      <TouchableOpacity style={[styles.fab, { bottom: 30 + insets.bottom }]} onPress={() => setIsPostModalOpen(true)}>
        ...
        <View style={styles.modalOverlay}>
          <SafeAreaView style={{ flex: 1 }}>
            <View style={styles.modalHeader}>
              <TouchableOpacity onPress={() => setIsPostModalOpen(false)} style={styles.modalHeaderBtn}><Ionicons name="close" size={24} color={COLORS.textMain} /></TouchableOpacity>
              ...
              <View style={styles.stationCard}>
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.lineScrollRow}>
                  <View style={{ flexDirection: 'row', gap: 8, paddingVertical: 2 }}>
                    {SUBWAY_LINES.filter(l => l !== '전체').map(line => (
                      <TouchableOpacity key={line} onPress={() => setPostLine(line)} style={[styles.lineChip, postLine === line && { backgroundColor: getLineColor(line), borderColor: getLineColor(line) }]}>
                ...
                <View style={styles.stationInputRow}>
                  <View style={[styles.lineCircleMd, { backgroundColor: postLine ? getLineColor(postLine) : '#C7C7CC' }]}>
                    <ThemedText style={styles.lineCircleMdText}>{postLine ? (postLine.match(/(\d+)/)?.[1] || postLine.slice(0, 2)) : '?'}</ThemedText>
                  </View>
                  <View style={styles.stationTextInputs}>
                    <TextInput style={styles.stationNameInput} placeholder="역 이름" placeholderTextColor={COLORS.textSub} value={station} onChangeText={setStation} />
                    <View style={styles.inputDivider} />
                    <TextInput style={styles.directionInput} placeholder="방면 (예: 교대)" placeholderTextColor={COLORS.textSub} value={direction} onChangeText={setDirection} />
                  </View>
                </View>
              </View>
              ...
              <View style={styles.photoRow}>
                ...
                {image && (
                  <View style={styles.photoPreviewWrap}>
                    ...
                    <TouchableOpacity style={styles.photoRemoveBtn} onPress={() => setImage(null)}><Ionicons name="close" size={12} color="white" /></TouchableOpacity>
                  </View>
                )}
              </View>
              <TouchableOpacity style={[styles.submitBtn, submitting && { opacity: 0.6 }]} onPress={submitReport} disabled={submitting}>
                {submitting ? <ActivityIndicator size="small" color="white" /> : <><Ionicons name="send" size={18} color="white" style={{ marginRight: 8 }} /><ThemedText style={styles.submitBtnText}>제보하기</ThemedText></>}
              </TouchableOpacity>
      ...
      <Modal visible={commentsReportId !== null} transparent animationType="slide" onRequestClose={closeComments}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.commentsSheet}>
          ...
          <View style={styles.commentsSheetHeader}>
            ...
            <TouchableOpacity onPress={closeComments}><Ionicons name="close" size={24} color={COLORS.textMain} /></TouchableOpacity>
          </View>
          {commentsLoading ? (
            <View style={styles.commentsLoadingContainer}><ActivityIndicator color={COLORS.primary} /></View>
          ) : (
            <FlatList data={comments} keyExtractor={(item) => item.id.toString()} style={styles.commentsList}
              ListEmptyComponent={<View style={styles.noComments}><Ionicons name="chatbubble-outline" size={36} color={COLORS.textSub} /><ThemedText style={styles.noCommentsText}>첫 댓글을 남겨보세요!</ThemedText></View>}
              renderItem={({ item }) => (
                <View style={styles.commentItem}>
                  <View style={styles.commentAvatar}><ThemedText style={styles.commentAvatarText}>{item.nickname?.[0] || '익'}</ThemedText></View>
          ...
          <View style={styles.commentInputRow}>
            <TextInput style={styles.commentInput} placeholder="댓글을 입력하세요..." placeholderTextColor={COLORS.textSub} value={commentText} onChangeText={setCommentText} returnKeyType="send" onSubmitEditing={submitComment} />
            <TouchableOpacity onPress={submitComment} style={styles.commentSendBtn} disabled={!commentText.trim() || commentSubmitting}>
              {commentSubmitting ? <ActivityIndicator size="small" color={COLORS.primary} /> : <Ionicons name="send" size={20} color={commentText.trim() ? COLORS.primary : COLORS.textSub} />}
            </TouchableOpacity>
          </View>
        </KeyboardAvoidingView>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  stationCard: { backgroundColor: 'white', borderRadius: 20, padding: 16, marginBottom: 12, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 10, elevation: 2 },
  ...
  noticeBanner: { flexDirection: 'row', alignItems: 'flex-start', backgroundColor: '#F2F2F7', borderRadius: 14, padding: 14, marginBottom: 20 },
  noticeText: { flex: 1, fontSize: 13, color: COLORS.textSub, lineHeight: 18 },
  titleInput: { backgroundColor: 'white', borderRadius: 14, paddingHorizontal: 16, paddingVertical: 14, fontSize: 15, color: COLORS.textMain, marginBottom: 10, borderWidth: 1.5, borderColor: COLORS.border },
  contentInput: { backgroundColor: 'white', borderRadius: 14, paddingHorizontal: 16, paddingVertical: 14, fontSize: 14, color: COLORS.textMain, height: 160, marginBottom: 16, borderWidth: 1.5, borderColor: COLORS.border, lineHeight: 22 },
  photoRow: { flexDirection: 'row', gap: 10, marginBottom: 24 },
  photoAddBtn: { width: 72, height: 72, borderRadius: 14, borderWidth: 1.5, borderColor: COLORS.border, borderStyle: 'dashed', justifyContent: 'center', alignItems: 'center', backgroundColor: 'white' },
  ...
  photoRemoveBtn: { position: 'absolute', top: -6, right: -6, width: 20, height: 20, borderRadius: 10, backgroundColor: '#333', justifyContent: 'center', alignItems: 'center' },
  submitBtn: { backgroundColor: COLORS.primary, borderRadius: 18, paddingVertical: 18, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' },
  submitBtnText: { fontSize: 17, fontWeight: '800', color: 'white' },
  modalBackdrop: { flex: 1, backgroundColor: 'rgba(0,0,0,0.4)' },
  commentsSheet: { backgroundColor: 'white', borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: '70%', paddingBottom: 20 },
